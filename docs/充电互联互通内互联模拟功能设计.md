...
- **收包安全流程**（已实现）：
  ...
  2. 使用**本地配置**（§6.2）中的 `SigSecret` 对 `OperatorID + Data + TimeStamp + Seq` 做 HMAC-MD5 **验签**
  3. 验签通过后用本地 `DataSecret` / `DataSecretIV` 做 AES-128-CBC（PKCS5/7）**解密** Data
  4. 业务处理后仍按**本地配置**加密并签名响应（`OperatorID` 取本地平台编码）
  5. **说明**：入站与出站均优先使用**本地配置**秘钥；出站时信封 `OperatorID` 为本地平台编码，对端按我方注册信息验签（§6.2、§6.3）
...
**秘钥用途**：本地配置用于本机 HTTP **服务平台**角色——接收请求时的验签、解密，以及应答时的加密、签名。对端第三方需使用与本地配置一致的秘钥向本机发起调用。
...
**秘钥用途**：三方配置主要提供对端**互联互通地址**与对端**平台编码**（`thirdParty.operatorId`）；本插件作为第三方客户端向外发起 HTTP 调用时，信封 `OperatorID` 与签名、加解密应使用**本地配置**（§6.2）中我方在对端注册的凭证——对端服务平台按该 OperatorID 查找秘钥验签。三方配置中的秘钥字段仅作本地秘钥未填时的兼容回退，**不用于**本机 HTTP 入站验签与解密。
