...
扫码启动由 **平台 / APP 扫描枪二维码** 后下发远程启动报文，模拟器按报文类型自动应答。

### 5.1 两种平台路径

平台扫码后可能下发不同报文（二选一由平台决定）：

| 平台下发 | 说明 | 侧栏流程（可选，仅用于配置应答） |
|----------|------|----------------------------------|
| `0x1F` | 账户/卡远程启动，常见于普通扫码 | **扫码远程启动流程**（配置 `0x21` 等） |
| `0x59` | VIN 扫码启动 | **扫码VIN启动流程**（配置 `0x5B` 模拟失败等） |

> **`0x59` / `0x1F` 均始终处理**，无需先选中对应侧栏流程。侧栏流程仅用于配置模拟失败等应答参数。

### 5.2 操作步骤

**扫码 VIN（`0x59`）前置条件（仅此两项）：**

1. 完成 **链接平台**（第 3 节），桩在线。
2. **连接车辆**，目标枪为 **链接** 状态且 VIN ≥ 8 位（第 4 节）。

之后由平台 / APP 扫码（或直接下发 `0x59`），模拟器自动交互。侧栏选 **扫码VIN启动流程**、打开 **启动控制 → 扫码** Tab 均非必须；扫码 Tab 仅便于查看枪二维码联调。

**扫码远程（`0x1F`）可参考步骤：**

1. 完成链接平台与连接车辆。
2.（可选）侧栏选 **扫码远程启动流程**，配置启动结果等。
3.（可选）打开 **启动控制 → 扫码** Tab 查看二维码。
4. 使用外部 APP 或平台工具扫描二维码，后续报文由模拟器自动交互。
...
充电成功后，若已打开 **启动控制** 弹窗会切换为 **充电信息**；可在侧栏 **桩订单** 查看订单（启动类型显示 **扫码启动** 或 **扫码VIN启动**）。
...
4. 打开 **启动控制** → **卡启动** Tab。
5. 输入 **卡号**（最多 16 位），点击 **卡启动**（进行中显示 **鉴证中…**）。
...
**Q：扫码后没反应？**  
A：确认桩 **在线**、目标枪为 **链接**（且扫码 VIN 时 VIN ≥ 8 位）。`0x59` / `0x1F` 无需先选侧栏流程即可处理；若需模拟失败应答，再到侧栏对应流程面板配置。
